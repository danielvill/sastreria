...
def admedi():
    ...
    if request.method == 'POST':
        id_medida = str(get_next_sequence('medidaId')).zfill(1)
        cliente = db["cliente"]
        ...
        exist_cliente = cliente.find_one({"id_cliente":id_cliente})
        
        if exist_cliente:
            flash("Datos ya ingresados no puede ingresar de nuevo" ,"alert")
            return render_template('cliente/in_medida.html')
        else:
            medi = Medida( id_medida ,id_cliente,id_cliente,busto,cintura,cadera,espalda,mangas,comentario )
            medidas.insert_one(medi.MedidaDBCollection())
            return redirect(url_for('medidas.envio'))
    else:
        return render_template('cliente/in_medida.html')
...
def edit_medi(edamedi):
    ...
    if busto and cintura  and cadera and espalda and mangas and comentario :
        ...
        return redirect(url_for('medidas.v_medida'))
    else:
        return render_template('cliente/medida.html')
