...
def edit_medi(edamedi):
    ...
    medida = medidas.find_one({'id_medida': edamedi, 'id_cliente': id_cliente})
    
    if not medida:
        flash("No tienes permisos para editar esta medida", "danger")
    ...
    if request.method == 'POST':
        ...
        if busto and cintura and cadera and espalda and mangas and comentario:
            medidas.update_one(
                {'id_medida': edamedi, 'id_cliente': id_cliente},
                {'$set': {
                    'busto': busto,
                    'cintura': cintura,
                    'cadera': cadera,
    ...
    return render_template('cliente/medida.html', medida=medida)
